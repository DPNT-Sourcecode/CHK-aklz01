...
function makeGroupOfferHandler(offer_config) {
    return function groupOfferHandler(sku_counts, context) {
        ...
        for (const [groupSkus, offer] of Object.entries(offer_config || {})) {
            ...
            // Apply offers as many times as possible
            let appliedCount = 0;
            while (groupItems.length >= requiredQuantity) {
                ...
                appliedCount += requiredQuantity;
            }

            // Update remaining counts
            if (appliedCount > 0) {
                let consumed = appliedCount;
                for (const sku of skus) {
                    if (consumed <= 0) break;
                    const available = remaining[sku] || 0;
                    const toConsume = Math.min(consumed, available);
                    remaining[sku] = available - toConsume;
                    consumed -= toConsume;
                }
            }
