...
class CheckoutSolution {
    ...
    applyMultiBuyOffers(sku_count) {
        const remaining_skus_after_offer = { ...sku_count };

        let total = 0;

        for (const [sku, offers] of Object.entries(MULTI_BUY_OFFERS)) {
            if (!remaining_skus_after_offer[sku]) {
                continue;
            }

            let quantity = remaining_skus_after_offer[sku];
            ...
            remaining_skus_after_offer[sku] = quantity;
        }

        return total, remaining_skus_after_offer;
    }
