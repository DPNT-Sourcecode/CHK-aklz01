'use strict';
const { GET_ONE_FREE_OFFERS, MULTI_BUY_OFFERS, PRICES } = require('./config');

class CheckoutSolution {
    constructor() {
        // configure offers engine with handlers in desired order
        const handlers = [
            makeGetOneFreeHandler(GET_ONE_FREE_OFFERS, PRICES),
            makeMultiBuyHandler(MULTI_BUY_OFFERS, PRICES)
        ];
        this.offersEngine = new OffersEngine(handlers, { PRICES, MULTI_BUY_OFFERS, GET_ONE_FREE_OFFERS });
    }

    // skus is expected to be a string
    checkout(skus) {
        if (skus.length === 0)
            return 0;

        if (typeof skus !== 'string') {
            return -1;
        }

        const count_per_sku = {};
        for (let sku of skus) {
            if (!PRICES.hasOwnProperty(sku))
                return -1;
            count_per_sku[sku] = (count_per_sku[sku] || 0) + 1;
        }

        let total = 0;

        const remaining_skus = this.applyGetOneFreeOffers(count_per_sku);

        const [multi_total, remaining_skus_after_offer] = this.applyMultiBuyOffers(remaining_skus);
        total += multi_total;

        for (const [sku, qty] of Object.entries(remaining_skus_after_offer)) {
            if (qty > 0) {
                total += qty * PRICES[sku];
            }
        }
        return total;
    }

    applyMultiBuyOffers(count_per_sku) {
        const remaining_skus = { ...count_per_sku };

        let total = 0;

        for (const [sku, offers] of Object.entries(MULTI_BUY_OFFERS)) {
            if (!remaining_skus[sku]) {
                continue;
            }

            let quantity = remaining_skus[sku];
            const sorted_offers = offers.sort((a, b) => b.quantity - a.quantity);

            for (const offer of sorted_offers) {
                const num_offers_applicable = Math.floor(quantity / offer.quantity);
                if (num_offers_applicable > 0) {
                    total += num_offers_applicable * offer.price;
                    quantity -= num_offers_applicable * offer.quantity;
                }
            }

            remaining_skus[sku] = quantity;
        }

        return [total, remaining_skus];
    }

    applyGetOneFreeOffers(count_per_sku) {

        const remaining = { ...count_per_sku }

        if (remaining['E'] >= 2 && remaining['B']) {
            const numE = remaining['E'];
            const freeBs = Math.floor(numE / 2);
            remaining['B'] = Math.max(0, remaining['B'] - freeBs);

        }
        return remaining;
    }
}

module.exports = CheckoutSolution;

